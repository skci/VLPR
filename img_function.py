...
class CardPredictor:
    ...
    def img_first_pre(self, car_pic_file):
        ...
        oldimg = img
        cv2.imwrite("tmp/oldimg.jpg", oldimg)
        ...
        cv2.imwrite("tmp/img_threshold.jpg", img_thresh)
    ...
    def img_only_color(self, filename, oldimg, img_contours):
        ...
        lower_blue = np.array([100, 110, 105])
        upper_blue = np.array([130, 255, 255])
        lower_yellow = np.array([12, 88, 99])
        upper_yellow = np.array([35, 255, 255])
        lower_green = np.array([18, 20, 132])
        ...
        for i, color in enumerate(colors):
            ...
            if color in ("blue", "yellow", "green"):
                ...
                try:
                    ...
                    cv2.imwrite("tmp/gray_img.jpg", gray_img)
                except:
                    print("gray转换失败")
                ...
                cv2.imwrite("tmp/chepai_xingtai.jpg", gray_img)
                ...
                for i, part_card in enumerate(part_cards):
                    ...
                    if charactor == "1" and i == len(part_cards) - 1:
                        if part_card_old.shape[0] / part_card_old.shape[1] > 8.39:  # 1太细，认为是边缘
                            continue
