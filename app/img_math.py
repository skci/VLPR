...
def img_Transform(car_contours, oldimg, pic_width, pic_hight):
    ...
    for car_rect in car_contours: #（中心(x,y), (宽,高), 旋转角度）
        ...
        if left_point[1] <= right_point[1]:  # 正角度，根据左右顶点判断
            ...
            dst = cv2.warpAffine(oldimg, M, (pic_width, pic_hight))  # 平移图像
            cv2.imwrite("tmp/img_dst.jpg", dst)
        ...
        elif left_point[1] > right_point[1]:  # 负角度，根据左右顶点判断
